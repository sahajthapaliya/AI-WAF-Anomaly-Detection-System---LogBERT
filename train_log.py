        ...
        for epoch in range(self.epochs):
            ...
            if avg_loss < best_loss:
                ...
                if self.hypersphere_loss:
                    ...
                    torch.save(best_radius, os.path.join(self.model_dir, "best_radius.pt"))
                    print("best radius", best_radius)
                    best_center_path = os.path.join(self.model_dir, "best_center.pt")
                    print("Save best center", best_center_path)
                    torch.save({"center": best_center, "radius": best_radius}, best_center_path)

                    total_dist_path = os.path.join(self.model_dir, "best_total_dist.pt")
                    print("save total dist: ", total_dist_path)
                    torch.save(total_dist, total_dist_path)
                    
                    torch.save(best_center, os.path.join(self.model_dir, "center.pt"))
                    torch.save(torch.tensor(best_radius), os.path.join(self.model_dir, "radius.pt"))
                    print("✅ Saved center.pt and radius.pt for prediction")
            else:
                epochs_no_improve += 1
